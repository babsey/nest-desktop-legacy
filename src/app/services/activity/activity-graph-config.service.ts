...
var STORAGE_NAME = 'activity-graph-config';


@Injectable({
  providedIn: 'root'
})
export class ActivityGraphConfigService {
  ...
  fromFiles(files: string[]): void {
    var configFiles = files.map(file => this._http.get('/assets/config/activity/' + file + '.json'));
