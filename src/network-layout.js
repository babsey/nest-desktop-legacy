...
function neuralNetwork(reference) {
    ...
    function dragstarted(d) {
        if (!d3Selection.event.active) simulation.alpha(1).restart();
        d.fx = d.x;
        d.fy = d.y;
    }

    function dragged(d) {
        if (!d3Selection.event.active) simulation.alpha(1);
        d.fx = d3Selection.event.x;
        d.fy = d3Selection.event.y;
    }

    function dragended(d) {
        if (!d3Selection.event.active) simulation.alpha(0);
        d.fx = null;
        d.fy = null;
    }
    ...
    layout.restart = function() {
        ...
        circle = circle.enter().append("g")
            ...
            .on('mouseover', function(d) {
                // enlarge target node
                d3Selection.select(this).select('circle').attr('transform', function(d) {
                    return 'scale(1.1)';
                });
            })
    ...
    function ticked() {
        ...
        circle.selectAll('circle').style('fill', function(d, i) {
            return (d === selected_node) ? d3Color.rgb(colors[d.index % 10]).brighter().toString() : "white";
        });
